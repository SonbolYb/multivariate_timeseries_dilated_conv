'''
Dilated convolution for the multidimensional delay vectors/ v1.1. individual
'''
...
PATH='./datasets'
MDPATH='./WISDM_v1.1/individual/models'
RESULT_PATH='./WISDM_v1.1/individual/results.csv'

feat_train=np.load(PATH+'/TRAIN_feat_in_v1_200.npy')
label_train=np.load(PATH+'/TRAIN_label_in_v1_200.npy')

feat_test=np.load(PATH+'/TEST_feat_in_v1_200.npy')
label_test=np.load(PATH+'/TEST_label_in_v1_200.npy')
...
out=conv2d(inp,weight=[3,20,1,32],bias=[32],padding='SAME',stride=1,dilation=[1,1,2,1],name='dil_1')
...
out=conv2d(out,weight=[1,4,32,32],bias=[32],padding='VALID',stride=4,dilation=[1,1,1,1],name='pool_4')
...
correct_pred=tf.cast(tf.equal(tf.argmax(pred,1),tf.argmax(target,1)),tf.float32)
