...
PATH='./datasets'
MDPATH='./WISDM_v2/models'
RESULT_PATH='./WISDM_v2/results.csv'

feat_train=np.load(PATH+'/TRAIN_feat_sp_v2_200.npy')
label_train=np.load(PATH+'/TRAIN_label_sp_v2_200.npy')

feat_test=np.load(PATH+'/TEST_feat_sp_v2_200.npy')
label_test=np.load(PATH+'/TEST_label_sp_v2_200.npy')
...
out=conv2d(out,weight=[1,2,32,32],bias=[32],padding='VALID',stride=2,dilation=[1,1,1,1],name='pool_2')
...
out=conv2d(out,weight=[3,3,32,64],bias=[64],padding='SAME',stride=1,dilation=[1,1,1,1],name='dil_5')
print("layer5_dil:\t",out.get_shape())

out=conv2d(out,weight=[1,2,64,64],bias=[64],padding='VALID',stride=2,dilation=[1,1,1,1],name='pool_6')
print("layer6_pool:\t",out.get_shape())


out_size=out.get_shape().as_list()
out_flat=tf.reshape(out,(out_size[0],-1))

out_flat=dense_layer(out_flat,shape_w=[out_size[1]*out_size[2]*out_size[3],512],shape_b=[512],name='dense')
out_flat=tf.nn.relu(out_flat)

out_final=dense_layer(out_flat,shape_w=[512,n_class],shape_b=[n_class],name='last')
